...
class WC_CPW_Hd_Repo {
	...
	public function __construct( $cryptoId, $mpk, $hdMode ) {
		...
		$this->hdMode	 = $hdMode;
		$this->tableName = WC_CPW_Loader::$hd_table;
	}

	public function insert( $address, $mpk_index, $status ) {
		WC_CPW_Logger::info( __FILE__, __LINE__, 'inserting ' . $address . ' into db as ' . $status );
		global $wpdb;
		$currentTime = time();
	...
	public function get_oldest_ready() {
		...
		WC_CPW_Logger::info( __FILE__, __LINE__, "Oldest ready address is: " . print_r( $address, true ) );
		return $address;
	}
	...
	public function set_total_received( $address, $totalReceived ) {
		global $wpdb;
		WC_CPW_Logger::info( __FILE__, __LINE__, 'Updating total received at ' . $address . ' to: ' . $totalReceived );
	...
	public function set_order_amount( $address, $orderAmount ) {
		global $wpdb;
		WC_CPW_Logger::info( __FILE__, __LINE__, 'Updating order amount at ' . $address . ' to: ' . $orderAmount );
	...
	public function set_status( $address, $status ) {
		global $wpdb;
		WC_CPW_Logger::info( __FILE__, __LINE__, 'Updating ' . $address . ' to ' . $status );
		if ( $status === 'assigned' ) {
			$currentTime = time();
	...
	public function set_order_id( $address, $orderId ) {
		global $wpdb;
		WC_CPW_Logger::info( __FILE__, __LINE__, 'Setting address ' . $address . ' order id to: ' . $orderId );
