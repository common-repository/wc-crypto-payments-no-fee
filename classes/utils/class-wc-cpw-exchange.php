...
class WC_CPW_Exchange {
	...
	public static function get_order_total_in_usd( $total, $fromCurr ) {
		...
		$apiKey = WC_CPW_Gateway::get_currency_converter_key();

		$response = wp_remote_get( 'https://freecurrencyapi.net/api/v2/latest?apikey=' . $apiKey );

		if ( is_wp_error( $response ) || $response[ 'response' ][ 'code' ] !== 200 ) {

			$response = wp_remote_get( 'http://www.floatrates.com/daily/usd.json' );

			if ( is_wp_error( $response ) || $response[ 'response' ][ 'code' ] !== 200 ) {
				throw new \Exception( __( 'Could not reach the currency conversion service. Please try again.', 'wc-crypto-payments' ) );
			}

			$body = json_decode( $response[ 'body' ] );

			$conversionRate	 = $body->{strtolower( $fromCurr )}->rate;
			set_transient( $transientKey, $conversionRate, 600 );
			$priceInUsd		 = $total * $conversionRate;

			return $priceInUsd;

			/* //don't use fixer.io , low limits for free key
		...
		$conversionRate	 = 1 / $body->{'data'}->{$fromCurr};
		set_transient( $transientKey, $conversionRate, 600 );
		$priceInUsd		 = $total * $conversionRate;
	...
	public static function get_cryptocompare_price( $cryptoId, $updateInterval ) {
		...
		if ( is_wp_error( $response ) || $response[ 'response' ][ 'code' ] !== 200 ) {
			...
			WC_CPW_Logger::info( __FILE__, __LINE__, print_r( $response, true ) );
			$response = wp_remote_get( 'https://min-api.cryptocompare.com/data/price?fsym=' . $cryptoId . '&tsyms=USD' );
			if ( is_wp_error( $response ) || $response[ 'response' ][ 'code' ] !== 200 ) {
		...
		//CPW_Logger::info( __FILE__, __LINE__, print_r( $response, true ) );
